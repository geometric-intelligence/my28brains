...
--> outputs in parameterized_meshes_dir
...
import my28brains.meshing.centering as centering
import my28brains.meshing.extraction as extraction
import my28brains.meshing.geodesics as geodesics
...
days_dir = [os.path.join(raw_data_dir, f"Day{i:02d}") for i in range(1, 61)]
...
if __name__ == "__main__":
    ...
    for hemisphere in default_config.hemisphere:
        hippocampus_centers = centering.center_whole_hippocampus_and_write(
            input_dir=meshed_data_dir, output_dir=centered_dir, hemisphere=hemisphere
        )
        for structure_id in default_config.structure_ids:
            if structure_id != -1:
                centering.center_substructure_and_write(
                    input_dir=meshed_data_dir,
    ...
    # Set up input paths for the next 2 steps
    ...
    for hemisphere, structure_id, area_threshold in itertools.product(
        ...
        output_dir = parameterized_meshes_dir
