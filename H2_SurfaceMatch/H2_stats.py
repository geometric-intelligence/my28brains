...
def H2KMean(samples, F0, a0, a1, b1, c1, d1, a2, paramlist):
    N = len(samples)
    samples = torch.from_numpy(samples).to(dtype=torch_dtype, device=torchdeviceId)
    mu = torch.mean(samples, axis=0)
    geods = (
        torch.cat((mu.repeat((N, 1, 1, 1)), samples.unsqueeze(dim=1)), dim=1)
        .cpu()
        .numpy()
    )
    print(geods.shape)
    for param in paramlist:
        ...
        geods, mu, F0 = KMeanIteration(
            mu, geods, samples, F0, newk, a0, a1, b1, c1, d1, a2, param
        )
        print(geods.shape)
    return geods, mu, F0
...
def KMeanIteration(mu, geods, samples, F, newk, a0, a1, b1, c1, d1, a2, param):
    ...
    midpoint = torch.from_numpy(midpoints[:, 1 : newk - 1, :, :]).to(
    ...
    def funopt(midpoint):
        mu = (
            torch.from_numpy(midpoint[0 : 3 * n])
            .to(dtype=torch_dtype, device=torchdeviceId)
            .reshape(n, 3)
        )
        midpoint = torch.from_numpy(midpoint[3 * n :].reshape(N, newk - 2, n, 3)).to(
            dtype=torch_dtype, device=torchdeviceId
        )
    ...
    def dfunopt(midpoint):
        ...
        Gmidpoint = np.concatenate(
            (
                Gmidpoint.detach().cpu().numpy().flatten().astype("float64"),
                Gmu.detach().cpu().numpy().flatten().astype("float64"),
            )
        )
        return Gmidpoint

    inp_vect = np.concatenate(
        (mu.cpu().numpy().flatten(), midpoint.cpu().numpy().flatten())
    )
    ...
    ngeods = np.concatenate(
        (
            np.tile(mu, (N, 1, 1, 1)),
            out[n * 3 :].reshape(N, newk - 2, n, 3),
            np.expand_dims(samples.cpu().numpy(), 1),
        ),
        axis=1,
    )
    geods = np.array(ngeods)
    return geods, torch.from_numpy(mu).to(dtype=torch_dtype, device=torchdeviceId), F


def H2UnparamKMean(
    samples, template, a0, a1, b1, c1, d1, a2, paramlist, N=None, geodesics=False
):
    if N is None:
        N = len(samples)
    ...
    for i in range(0, N):
        ...
        geod, F0 = H2StandardIterative(
            [mu, F0], sample, a0, a1, b1, c1, d1, a2, paramlist
        )
        k = geod.shape[0]
...
def H2_UnparamPCA(
    ...
    if geods is None:
        for i in range(0, len(samples)):
            print(
                "Computing Tangent Vector in the direction of sample {}/{}".format(
                    i + 1, N
                )
            )
            geod, F0 = H2StandardIterative(
    ...
    V = torch.from_numpy(V0).to(dtype=torch_dtype, device=torchdeviceId)
    M = torch.from_numpy((T - mean(T, axis=0))).to(
        dtype=torch_dtype, device=torchdeviceId
    )
    ...
    for i in range(0, M.shape[0]):
        for j in range(i, M.shape[0]):
            cov[i, j] = getH2Metric(
                V, M[i, :, :], M[j, :, :], a0, a1, b1, c1, d1, a2, F_sol
            )
            cov[j, i] = cov[i, j]
    ...
    if tol is not None:
        for i in range(0, evalue.shape[0]):
            ...
            print(evector.shape)
            if evalue[i] > tol:
                PC1p, F = H2InitialValueProblem(
                    V0, evalues[i] * evector, k, a0, a1, b1, c1, d1, a2, F0
                )
                PC1n, F = H2InitialValueProblem(
                    V0, -1 * evalues[i] * evector, k, a0, a1, b1, c1, d1, a2, F0
                )
...
def H2PCA(
    ...
    if geods is None:
        for i in range(0, len(samples)):
            ...
            geod, F0 = H2Parameterized(
                [V0, F0], [samples[i], F0], a0, a1, b1, c1, d1, a2, paramlist
            )
            k = geod.shape[0]
            T[i, :] = geod[1] - geod[0]
    else:
