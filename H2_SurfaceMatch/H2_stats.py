...
def KMeanIteration(mu,geods,samples,F,newk,a0,a1,b1,c1,d1,a2,param):
    ...
    midpoint = torch.from_numpy(midpoints[:,1:newk-1,:,:]).to(dtype=torch_dtype, device=torchdeviceId)
    ...
    def funopt(midpoint):
        mu=torch.from_numpy(midpoint[0:3*n]).to(dtype=torch_dtype, device=torchdeviceId).reshape(n,3)
        midpoint=torch.from_numpy(midpoint[3*n:].reshape(N,newk-2,n,3)).to(dtype=torch_dtype, device=torchdeviceId)
...
def H2_UnparamPCA(V0,samples,F0,a0,a1,b1,c1,d1,a2,paramlist,components=1,tol=None, geods=None,st_devs=1):
    ...
    M = torch.from_numpy((T-mean(T,axis=0))).to(dtype=torch_dtype, device=torchdeviceId)    
