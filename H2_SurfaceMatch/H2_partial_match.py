...
def SymmetricH2Matching_W(source,target,geod,F_init,Rho_init,a0,a1,b1,c1,d1,a2,param):    
    ...
    FunS = torch.from_numpy(np.ones((int(np.size(source[0])/3),))).to(dtype=torch_dtype, device=torchdeviceId)
    FunT = torch.from_numpy(np.ones((int(np.size(target[0])/3),))).to(dtype=torch_dtype, device=torchdeviceId)
    
    Fun_sol = torch.from_numpy(np.ones((int(geod.shape[1]),))).to(dtype=torch_dtype, device=torchdeviceId)
    ...
    def funopt(geod):
        ...
        geod=torch.from_numpy(ngeod.reshape(N,n,3)).to(dtype=torch_dtype, device=torchdeviceId)
