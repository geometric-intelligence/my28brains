...
from enr.H2 import *
from H2_match import H2Midpoint, SymmetricH2Matching
...
def SymmetricH2Matching_W(
    source, target, geod, F_init, Rho_init, a0, a1, b1, c1, d1, a2, param
):
    # Read out parameters
    ...
    VS = torch.from_numpy(source[0]).to(dtype=torch_dtype, device=torchdeviceId)
    VT = torch.from_numpy(target[0]).to(dtype=torch_dtype, device=torchdeviceId)
    ...
    geod = torch.from_numpy(geod).to(dtype=torch_dtype, device=torchdeviceId)
    F_sol = torch.from_numpy(F_init).to(dtype=torch.long, device=torchdeviceId)
    Rho_sol = torch.from_numpy(Rho_init).to(dtype=torch_dtype, device=torchdeviceId)
    B_sol = torch.from_numpy(computeBoundary(F_init)).to(
        dtype=torch.bool, device=torchdeviceId
    )

    N = geod.shape[0]
    n = geod.shape[1]

    FunS = torch.from_numpy(np.ones((int(np.size(source[0]) / 3),))).to(
        dtype=torch_dtype, device=torchdeviceId
    )
    FunT = torch.from_numpy(np.ones((int(np.size(target[0]) / 3),))).to(
        dtype=torch_dtype, device=torchdeviceId
    )

    Fun_sol = torch.from_numpy(np.ones((int(geod.shape[1]),))).to(
        dtype=torch_dtype, device=torchdeviceId
    )

    sig_geom = torch.tensor([sig_geom], dtype=torch_dtype, device=torchdeviceId)
    sig_grass = torch.tensor([sig_grass], dtype=torch_dtype, device=torchdeviceId)
    sig_fun = torch.tensor([sig_fun], dtype=torch_dtype, device=torchdeviceId)

    # Define Energy and set parameters
    energy = enr_match_H2_sym_w(
        VS,
        FS,
        FunS,
        VT,
    ...
    def funopt(geod):
        ...
        geod = torch.from_numpy(ngeod.reshape(N, n, 3)).to(
            dtype=torch_dtype, device=torchdeviceId
        )
        Rho = torch.from_numpy(Rho).to(dtype=torch_dtype, device=torchdeviceId)
    ...
    def dfunopt(geod):
        ...
        Ggeod = np.concatenate(
            (
                Ggeod.detach().cpu().numpy().flatten().astype("float64"),
                GRho.detach().cpu().numpy().flatten().astype("float64"),
            )
        )
        return Ggeod

    inp_vect = np.concatenate(
        (geod.cpu().numpy().flatten(), Rho_sol.cpu().numpy().flatten())
    )
...
def WeightUpdate(source, target, geod, F_init, Rho_init, param):
    ...
    FS = torch.from_numpy(source[1]).to(dtype=torch.long, device=torchdeviceId)
    FT = torch.from_numpy(target[1]).to(dtype=torch.long, device=torchdeviceId)
    V_sol = torch.from_numpy(geod[N - 1]).to(dtype=torch_dtype, device=torchdeviceId)
    F_sol = torch.from_numpy(F_init).to(dtype=torch.long, device=torchdeviceId)
    Rho_sol = torch.from_numpy(Rho_init).to(dtype=torch_dtype, device=torchdeviceId)
    B_sol = torch.from_numpy(computeBoundary(F_init)).to(
        dtype=torch.bool, device=torchdeviceId
...
def H2MultiRes(
    ...
    for j in range(0, iterations):
        ...
        if partial:
            geod, Rho0, ener, Dic = SymmetricH2Matching_W(
                sources[index],
                targets[index],
                geod,
                F0,
            ...
            geod, ener, Dic = SymmetricH2Matching(
                sources[index], targets[index], geod, F0, a0, a1, b1, c1, d1, a2, params
            )
        ...
        if tri_upsample:
            ...
            geod_subi, F_Subi, RhoSub = io.subdivide_mesh(
                geod[0], F0, Rho=Rho0, order=1
            )
            Rho0 = RhoSub
