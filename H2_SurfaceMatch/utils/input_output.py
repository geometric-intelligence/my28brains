...
def plotMatchingResult(
    source, matched_target, target, matching_type, matched_source=None
):
    ...
    if matching_type == "Symmetric" and (matched_source is not None):
        ...
        matched_source = getMeshFromData(matched_source).translate(
            (5, 0, 0), relative=False
        )
        matched_target = getMeshFromData(matched_target).translate(
            (10, 0, 0), relative=False
        )
        target = getMeshFromData(target).translate((15, 0, 0), relative=False)
        o3d.visualization.draw_geometries(
            [source, matched_source, matched_target, target]
        )
