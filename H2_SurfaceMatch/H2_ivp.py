...
def stepforward(V0, V1, a0, a1, b1, c1, d1, a2, F_init):
    ...
    def funopt(V2):
        V2 = torch.from_numpy(V2.reshape(N, 3)).to(
...
def H2InitialValueProblem(
    V0, h, steps, a0, a1, b1, c1, d1, a2, F_init, total_steps=None
):
    if total_steps is None:
        total_steps = steps
    V0 = torch.from_numpy(V0).to(dtype=torch_dtype, device=torchdeviceId)
    h = torch.from_numpy(h).to(dtype=torch_dtype, device=torchdeviceId)
    ...
    for i in range(2, total_steps):
        ...
        V2 = torch.from_numpy(V2).to(dtype=torch_dtype, device=torchdeviceId)
