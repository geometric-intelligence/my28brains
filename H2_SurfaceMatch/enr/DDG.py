...
def getSurfMetric(V, F, device=None):
    ...
    alpha = torch.zeros((nF, 3, 2)).to(dtype=torch_dtype, device=device)
...
def getMeshOneForms(V, F, device=None):
    """Computation of the Riemannian metric evaluated at the faces of a triangulated surface.
...
def getLaplacian(V, F, device=None):
    ...
    def L(h):
        ...
        Lh = torch.zeros((nV, 3)).to(dtype=torch_dtype, device=device)
...
def getVertAreas(V, F, device=None):
    ...
    idx = F.view(-1)
    incident_areas = torch.zeros(nV, dtype=torch_dtype, device=device)
...
def computeBoundary(F):
    ...
    vals = np.ones(3 * nF, dtype=np.int32)  # was np.int
    E = scipy.sparse.coo_matrix((vals, (rows, cols)), shape=(nV, nV))
    E -= E.transpose()
    i, j = E.nonzero()
    BoundaryIndicatorOfVertex = np.zeros(nV, dtype=bool)  # was np.bool
