...
def SRCF(F, B, V):
    ...
    VectorMeanCurvatureOfVertex = torch.zeros(V.shape).to(
        dtype=V.dtype, device=V.device
    )
    VectorMeanCurvatureOfVertex.index_add_(
        0, F[:, 0], torch.cross(V[F[:, 2], :] - V[F[:, 1], :], UnitNormalVectorOfFace)
    )
    VectorMeanCurvatureOfVertex.index_add_(
        0, F[:, 1], torch.cross(V[F[:, 0], :] - V[F[:, 2], :], UnitNormalVectorOfFace)
    )
    VectorMeanCurvatureOfVertex.index_add_(
        0, F[:, 2], torch.cross(V[F[:, 1], :] - V[F[:, 0], :], UnitNormalVectorOfFace)
    )
    ...
    **objfun
    ...
    def energy(x):
        ...
        if weight_MCV > 0:
            Hx = SRCF(FS, BS, x)
            return (
                weight_SRNF * SRNF_cost(nS, nx)
                + weight_coef_dist * dataloss(x)
                + weight_MCV * ((Hx - HS) ** 2).sum(axis=1).sum()
            )
        else:
    ...
    **objfun
    ...
    def energy(x, y):
        ...
        if weight_MCV > 0:
            ...
            return (
                weight_SRNF * SRNF_cost(nx, ny)
                + weight_MCV * ((Hx - Hy) ** 2).sum(axis=1).sum()
                + weight_coef_dist_S * dataloss_S(x)
                + weight_coef_dist_T * dataloss_T(y)
            )
        else:
            E = 0
