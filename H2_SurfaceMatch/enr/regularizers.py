...
def quartic_pen(m0=0., m1=1., h=0.5, clip_epsilon=0.25, clip_grad=0.5, normalization=True):
    ...
    C = torch.from_numpy(np.linalg.solve(X,y)).to(dtype=torch_dtype, device=torchdeviceId)
    ...
    def penalty(V, F, Rho):
        ...
        Q = torch.from_numpy(np.zeros(nV,1)).to(dtype=torch_dtype, device=torchdeviceId)
