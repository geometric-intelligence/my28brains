...
def quartic_pen(
    m0=0.0, m1=1.0, h=0.5, clip_epsilon=0.25, clip_grad=0.5, normalization=True
):
    ...
    C = torch.from_numpy(np.linalg.solve(X, y)).to(
    ...
    def penalty(V, F, Rho):
        ...
        Q = torch.from_numpy(np.zeros(nV, 1)).to(
            dtype=torch_dtype, device=torchdeviceId
        )
        ...
        clip_mid = torch.from_numpy(
            np.logical_and(
                (Rho <= m1 + clip_epsilon).cpu().numpy(),
                (Rho >= m0 - clip_epsilon).cpu().numpy(),
            )
        ).to(dtype=torch.bool, device=torchdeviceId)
