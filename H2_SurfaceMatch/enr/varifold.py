...
def lossVarifoldSurf(FS, FunS, VT, FT, FunT, K):
    def CompCLNn(F, V, Fun):
        if F.shape[1] == 2:
            ...
            Fun0, Fun1, Fun2 = (
                Fun.index_select(0, F[:, 0]),
                Fun.index_select(0, F[:, 1]),
                Fun.index_select(0, F[:, 2]),
            )
            C, N, Fun_F = (
                (V0 + V1 + V2) / 3,
                0.5 * torch.cross(V1 - V0, V2 - V0),
                (Fun0 + Fun1 + Fun2) / 3,
            )
    ...
    def loss(VS):
        ...
        return (
            cst
            + K(CS, CS, NSn, NSn, Fun_FS, Fun_FS, LS, LS)
            - 2 * K(CS, CT, NSn, NTn, Fun_FS, Fun_FT, LS, LT)
        )
...
def VKerenl(kernel_geom, kernel_grass, kernel_fun, sig_geom, sig_grass, sig_fun):
    ...
    def K(x, y, u, v, f, g, r1, r2):
        d = x.shape[1]
        pK = Genred(
            expr_geom + "*" + expr_grass + "*" + expr_fun + "*r",
            [
                "a=Pm(1)",
                "b=Pm(1)",
                "c=Pm(1)",
