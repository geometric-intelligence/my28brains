...
def StandardMatching(source, target, target_init, param):
    ...
    BS = torch.from_numpy(computeBoundary(source[1])).to(
    ...
    if len(source) < 3 or use_fundata == 0:
        ...
        FunS = torch.from_numpy(source[2]).to(dtype=torch_dtype, device=torchdeviceId)
    ...
    if len(target) < 3 or use_fundata == 0:
        ...
        FunT = torch.from_numpy(target[2]).to(dtype=torch_dtype, device=torchdeviceId)
    ...
    energy = enr_match_SRNF(
        ...
        FT,
        FunT,
        weight_coef_dist=weight_coef_dist_T,
        weight_MCV=weight_MCV,
    ...
    def funopt(z):
        n = int(len(z) / 3)
        x = z.reshape(n, 3)
        x = torch.from_numpy(x).to(dtype=torch_dtype, device=torchdeviceId)
...
def SymmetricMatching(source, target, source_init, target_init, param):
    ...
    B_sol = torch.from_numpy(computeBoundary(source_init[1])).to(
        dtype=torch.bool, device=torchdeviceId
    )
    ...
    if len(source_init) < 3 or use_fundata == 0:
        Fun_sol = torch.from_numpy(np.zeros((int(np.size(source_init[0]) / 3),))).to(
            dtype=torch_dtype, device=torchdeviceId
        )
    else:
        Fun_sol = torch.from_numpy(source_init[2]).to(
            dtype=torch_dtype, device=torchdeviceId
        )
    ...
    def funopt(z):
        ...
        x = torch.from_numpy(x).to(dtype=torch_dtype, device=torchdeviceId)
        y = torch.from_numpy(y).to(dtype=torch_dtype, device=torchdeviceId)
...
def MultiResMatching(source, target, source_init, target_init, paramlist):
    ...
    for i in range(0, NoRuns):
        print("Iteration: ", i)
        param = paramlist[i]
        f0[i], f1[i], En[i], Dic[i] = SymmetricMatching(
            source, target, source_init, target_init, param
        )
...
def SRCF_dist_square(source, target):
    ...
    BT = torch.from_numpy(computeBoundary(target[1])).to(
        dtype=torch.bool, device=torchdeviceId
    )
    HS = SRCF(FS, BS, VS)
...
def SRNF_inversion(Q, source_init, max_iter):
    ...
    Q = torch.from_numpy(Q).to(dtype=torch_dtype, device=torchdeviceId)
