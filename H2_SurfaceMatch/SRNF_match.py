...
# Output: optimal vertices f0 and f1.
    ...
    if len(source)<3 or use_fundata==0:
        FunS = torch.from_numpy(np.zeros((int(np.size(source[0])/3),))).to(dtype=torch_dtype, device=torchdeviceId)
    else:
        FunS = torch.from_numpy(source[2]).to(dtype=torch_dtype, device=torchdeviceId)

    if len(target)<3 or use_fundata==0:
        FunT = torch.from_numpy(np.zeros((int(np.size(target[0])/3),))).to(dtype=torch_dtype, device=torchdeviceId)
    else:
...
# Output: optimal vertices f0 and f1.
    ...
    if len(source_init)<3 or use_fundata==0:
        Fun_sol = torch.from_numpy(np.zeros((int(np.size(source_init[0])/3),))).to(dtype=torch_dtype, device=torchdeviceId)
    else:
        Fun_sol= torch.from_numpy(source_init[2]).to(dtype=torch_dtype, device=torchdeviceId)
