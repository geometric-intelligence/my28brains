...
def generate_twisted_cube_mesh():
    """Generate a twisted cube mesh."""
    vertices = np.array(
        [
            [0, 2, 2],
            [2, 0, 2],
...
def add_linear_noise(space, y, dataset_name, project_linear_noise, noise_factor=0.01):
    ...
    if dataset_name == "synthetic_mesh" and project_linear_noise:
        y_noisy_projected = []
        for point in y_noisy:
            point_mean = gs.mean(point)
            point -= point_mean
            y_noisy_projected.append(space.projection(point))
        y_noisy = gs.array(y_noisy_projected)
    elif project_linear_noise:  # hypersphere or hyperboloid
