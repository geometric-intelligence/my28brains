...
def H2MultiRes(
    ...
    for i in range(0, resolutions):
        print(f"FS decimation target: {int(FS.shape[0] / 4)}")
        print(f"FT decimation target: {int(FT.shape[0] / 4)}")

        [VS, FS] = io.decimate_mesh(VS, FS, int(FS.shape[0] / 4))  # was 4
        sources = [[VS, FS]] + sources
        [VT, FT] = io.decimate_mesh(VT, FT, int(FT.shape[0] / 4))  # was 4
    ...
    for j in range(0, iterations):
        ...
        if tri_upsample:
            ...
            F_Sub = []
            for i in range(0, N):
                print(f"iteration {i} in range(N) to subdivice mesh")
